module GiavaScript
  module StatementParserShared
    ...
    private def find_simple_statement_end_index(index : Int32, stop_before_else : Bool = false) : Int32
      current = index
    ...
    private def find_function_end_index(index : Int32, error_message : String = "Error: invalid function definition") : Int32
      header_end_index = find_function_header_end_index(index, error_message)
      raise ExpressionError.new(error_message) unless @source[header_end_index]? == '{'

      find_matching_brace_end_index(header_end_index, error_message) + 1
    end

    private def find_function_header_end_index(index : Int32, error_message : String) : Int32
      ...
      current = skip_whitespace(current)
      raise ExpressionError.new(error_message) unless @source[current]? == '('
